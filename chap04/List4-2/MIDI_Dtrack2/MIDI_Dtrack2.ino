...
void doNote(byte note, byte velocity, int down){
  ...
  if(down == LOW) noteSend(0x80,note,velocity); // note off
  else  noteSend(0x90,note,velocity); // note on

}
 void noteSend(byte cmd, byte data1, byte data2) {
  cmd = cmd | byte((channel +1) & 0xf);  // next channel number up
  Serial.write(cmd);
  Serial.write(data1);
  Serial.write(data2);
}
