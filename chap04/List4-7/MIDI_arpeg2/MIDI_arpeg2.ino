 ...
 * ----------------- 
 * Repeates all notes held down note interval fixed
 * uses switches to enable any missing step
 * and LEDs to display when they are on. 
*/

#define bLength 40
  byte inPins[] = {12, 10, 8, 6, 4, 2, 15, 16};
  byte outPins[] = {13, 11, 9, 7, 5, 3, 18, 19};
  byte seqLength = 8;
 //variables setup
...
void checkIn(){
  ...
  if (Serial.available() > 0) {
    ...
    Serial.write(incomingByte); // make thru everything recieved
...
void checkOut(){ // see if we need to send anything from the buffer
 static int place =0;
 static int lastStep = 0, arpStep = 0;
 int count = 0;
 if(millis() > time){
   place++;
 ...
 if(count < bLength) { // found next note to output
    arpStep++;
    if(arpStep >= seqLength) arpStep = 0;
    digitalWrite(outPins[lastStep],LOW); // turn last LED off
    if(digitalRead(inPins[arpStep])){
     noteSend(0x80, storeNote[place], 0); // turn previous note off
     noteSend(0x90, storeNote[place], storeVel[place]); // trigger note again
     digitalWrite(outPins[arpStep],HIGH);
    }
         lastStep = arpStep;
   }
 }
