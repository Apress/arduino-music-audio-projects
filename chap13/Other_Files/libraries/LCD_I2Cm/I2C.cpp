...
void I2C::setSpeed(uint8_t _fast)
{
 ...
 if(_fast == 2)
  {
    TWBR = ((F_CPU / 800000) - 16) / 2;
  }
if(_fast == 3)
  {
    TWBR = ((F_CPU / 1000000) - 16) / 2;
  }
...
uint8_t I2C::write2(uint8_t address, uint8_t registerAddress, uint8_t registerAddress2, uint8_t *data, uint16_t numberBytes)
{
  returnStatus = 0;
  returnStatus = start();
  if(returnStatus){return(returnStatus);}
  ...
  returnStatus = sendByte(registerAddress2);
  if(returnStatus)
  {
    if(returnStatus == 1){return(3);}
    return(returnStatus);
